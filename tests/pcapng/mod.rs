...
fn test_custom_block() {
    ...
    impl CustomCopiable<'_> for MyCustomPayload {
        ...
        fn from_slice(slice: &[u8]) -> Result<MyCustomPayload, PcapError> {
            let mut cursor = std::io::Cursor::new(slice);
            let magic_number = cursor.read_u64::<BigEndian>()?;
            Ok(MyCustomPayload { magic_number })
        }
    }
    ...
    let block_to_write = original_payload.clone().into_custom_block().into_block();
    pcapng_writer
