...
fn writer_roundtrip_preserves_negative_timestamp_offset() {
    ...
    writer
        .write_block(&interface.into_block())
        .expect("Failed to write interface description block");
    writer
        .write_block(&packet.into_block())
        .expect("Failed to write enhanced packet block");

    let mut reader = PcapNgReader::new(&buffer[..]).expect("Failed to create reader");

    let (interface_block, _) = reader
        .next_block()
        .expect("Missing interface block")
        .expect("Failed to read interface block");
    ...
    let (packet_block, _) = reader
        .next_block()
        .expect("Missing packet block")
        .expect("Failed to read packet block");
    match packet_block {
        Block::EnhancedPacket(block) => {
            assert_eq!(
                block.timestamp, -1_000_000_000,
                "Pre-epoch timestamp did not survive round-trip"
            );
        }
