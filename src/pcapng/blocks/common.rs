...
pub(crate) trait PcapNgOption {
    ...
    fn from_slice(code: u16, length: u16, data: &[u8]) -> Result<Self, PcapError>;
    fn write_to<B: ByteOrder, W: Write>(&self, writer: &mut W) -> IoResult<usize>;
}

pub(crate) trait WritePcapNgOpt {
    fn write_opt<B: ByteOrder, W: write>(&self, code: u16, writer: &mut W);
}

impl<'a> WritePcapNgOpt for Cow<'a, [u8]> {

    fn write_opt<B: ByteOrder, W: write>(&self, code: u16, writer: &mut W) -> IoResult<usize> {
...
impl<'a> WritePcapNgOpt for Cow<'a, str> {
