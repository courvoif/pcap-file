...
impl<'a> CustomBlock<'a, true> {
    ...
    pub fn interpret<T: CustomCopiable<'a>>(&'a self) -> Result<T, PcapError> {
        if self.pen != T::PEN {
            return Err(PcapError::InvalidPEM(T::PEN, self.pen));
        }
...
impl<'a> CustomBlock<'a, false> {
    ...
    pub fn interpret<T: CustomNonCopiable<'a>>(&'a self, state: &T::State) -> Result<T, PcapError> {
        if self.pen != T::PEN {
            return Err(PcapError::InvalidPEM(T::PEN, self.pen));
        }
...
/* ----- impl PcapNgBlock for CustomBlock ----- */
...
pub trait CustomCopiable<'a> {
    ...
    fn from_slice(slice: &'a [u8]) -> Result<Self, PcapError>
    where
        Self: Sized;

    /// Write this payload into a writer.
    fn write_to<W: Write>(&self, writer: &mut W) -> Result<usize, PcapError>;

    /// Convert this block in a copiable [`CustomBlock`]
    fn into_custom_block(self) -> CustomBlock<'a, true>
    where
        Self: std::marker::Sized,
    {
        let mut data = Vec::new();
        let _ = self.write_to(&mut data);

        CustomBlock { pen: Self::PEN, payload: Cow::Owned(data) }
    }
}
...
pub trait CustomNonCopiable<'a> {
    ...
    /// External state that can be needed in order to parse/write the block.
    type State;

    /// Try to parse this payload from a slice.
    fn from_slice(state: &Self::State, slice: &'a [u8]) -> Result<Self, PcapError>
    where
        Self: Sized;

    /// Write this payload into a writer.
    fn write_to<W: Write>(&self, state: &Self::State, writer: &mut W) -> Result<usize, PcapError>;

    /// Convert this block in a non-copiable [`CustomBlock`]
